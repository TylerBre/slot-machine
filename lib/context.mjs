...
// v0 (legacy, no `v`) -> v1: stamp version, default the two top-level containers.
// v1 -> v2: add empty settings + agents containers; repos already conform (new fields optional).
const CONFIG_ELEVATORS = [
  raw => ({ v: 1, current: raw.current ?? null, repos: raw.repos ?? {} }),
  raw => ({ ...raw, v: 2, settings: raw.settings ?? {}, agents: raw.agents ?? {} }),
];
...
/**
 * The canonical persisted config shape: v/current/settings/agents/repos. Pure - no IO.
 * @param {object} cfg - the config to stamp.
 * @returns {object} the stamped config with all keys present and defaulted.
 */
 ...
 * @param {object} cfg - the config to write ({ current, repos, settings, agents }); the current `v` is stamped on.
 */
export function saveConfig(cfg) {
  const stamped = stampConfig(cfg);
...
function repoFlag(argv) {
  // Only scan BEFORE a '--' terminator, so a guarded literal message ('sm msg send -- --repo x')
  // is not mistaken for a repo selection - matching how the router strips --repo.
  const term = argv.indexOf('--');
  const scan = term >= 0 ? argv.slice(0, term) : argv;
  const idx = scan.indexOf('--repo');
  if (idx >= 0 && scan[idx + 1])
    return scan[idx + 1];
  const eq = scan.find(arg => arg.startsWith('--repo='));
