...
import { resolveActive, loadConfig, CONFIG_FILE } from './context.mjs';
...
export const SECTIONS = {
  ...
  worker: `worker - the Claude processes in the slots
  ...
  kill SLOT               end a worker's process; its pane falls back to a shell
  ...
  lock: `lock - slot + shared-resource locks
  ls                      list held resource locks (holder, age, task)
  claim NAME [task]       lock a slot OR a shared resource (e.g. browser)
...
export const USAGE = `sm - slot machine: tmux + git-worktree orchestration for your Claude worker fleet

Point sm at a git repo; the repo's sibling worktrees are its "slots". sm builds a tmux
session with one Claude worker pane per slot plus the "desk" window you dispatch from,
...
Examples
  ...
  'slot create': `sm slot create LABEL [base]
...
shell you dispatch from; the rest run Claude with N panes each, one pane per slot worktree.
...
Examples
  ...
  'msg send': `sm msg send MESSAGE [-s SPEC | -f] [--brief] [-t SESSION]

Type one submitted line into slot panes, then exit (builds nothing). By default it goes to
every slot; target a subset with -s, or the first reusable slot with -f. MESSAGE is sent
literally, so a leading '/' hits a Claude slash-command; a message starting with '-' needs a
...
Options
  ...
  --json             print what was sent as JSON
...
slot. Columns: slot | status | [worker] | pr | branch. The worker column shows only when a
slot's Claude has died (its pane fell back to a shell).
...
Options
  ...
  --watch      live-refresh the table every 5s (Ctrl-C to stop)
...
Options
  --watch   live-refresh every 5s (Ctrl-C to stop)
...
Options
  ...
  -f, --follow    re-render every 2s (Ctrl-C to stop)
...
later 'claude -c' in that pane resumes it. Kill sessions with 'sm session kill'.
...
Examples
  ...
  'slot reset': `sm slot reset SLOT [--force]

Return a slot to a clean base branch at origin/<base> - how you reclaim a merged slot for new
work. Refuses a slot held by a live session or with uncommitted changes unless --force.

Options
  --force   reset even if live-locked or dirty (discards uncommitted changes)
  --json    print the result as JSON
...
Lock a slot or a shared resource. A NAME matching a slot worktree writes its .worktree-lock
(mark busy - worker run / msg send do this automatically). Any other name is a MACHINE-LEVEL RESOURCE
lock (e.g. 'browser' for the shared authenticated Playwright browser): acquisition is atomic,
so two claimants can never both win - the loser sees who holds it. Workers: claim the resource
BEFORE using it, release it right after; that replaces racing/killing each other for it.
...
Options
  -s, --slot X      record which slot holds a resource (default: auto-detected from cwd)
  -w, --wait        queue: retry until the resource frees (up to 10 min)
  --force           steal the lock (also auto-steals when the holder's worker is dead)
  -t, --session S   record the tmux session (informational)
...
Free a slot's .worktree-lock or a named resource lock - the inverse of claim. 'sm slot reset'
also releases the slot lock as part of reclaiming a slot.
...
Examples
  ...
  'msg inbox': `sm msg inbox [--watch] [--clear]
...
~/.local/bin, PATH, MCP registration with Claude Code, the resolved repo config and where
...
Kill tmux session(s). Ends the panes' Claude processes; the conversations survive on disk and
resume on the next 'sm session create'.
...
You supervise; you do not do feature work yourself. Each slot holds one worker (a Claude
process) doing one task in its own git worktree. Your job is to keep them fed and unblocked.
...
Your loop:
  ...
  3. Check in        sm msg inbox             (reports workers sent via sm msg report - read first)
...
export const ROLE_WORKER = `You are a WORKER - the Claude process for one worktree slot. Do the assigned task here,
and here only.
  ...
  - Report back with  sm msg report "<msg>"  - it is part of DONE. Report when you finish
