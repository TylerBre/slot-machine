...
import { cmdDoctor, cmdFloor, cmdJournal, cmdStats, cmdVersion } from './commands/top.mjs';
...
export async function run(argv) {
  ...
  process.on('exit', (code) => {
    recordUsage({
      cmd: usageCmd ?? '(none)',
      // watch's exit 3 is a defined outcome ("nothing to report"), not a failure - stats
      // must not read a quiet fleet as a broken command. Exit 2 on the --hook path is a
      // DELIVERED Stop-hook block (the hook protocol's blocking convention), not an error.
      ok: code === 0
        || (code === 3 && (usageCmd ?? '').startsWith('watch'))
        || (code === 2 && usageCmd === 'watch --hook'),
      ms: Date.now() - t0,
      repo: REPO_NAME,
  ...
  const main = async () => {
    ...
    if (isWatch)
      ...
      usageCmd += ' --wait';
    // sm watch: bucket hook invocations, one-shot checks, and the blocking mode apart -
    // hooks fire on every desk turn, and the loop's runtime is idle waiting, not command
    // latency; either would poison the others' avg/max in sm stats.
    else if (route === 'watch')
      usageCmd += rest.includes('--hook') ? ' --hook' : rest.includes('--check') ? ' --check' : ' --loop';
    // session create/attach hold the terminal for the whole attachment - that duration is
    // not command latency (the desk saw 'avg 69 hours'); bucket them apart like the waits.
    else if (route === 'session create' || route === 'session attach')
      usageCmd += ' --attached';
    // serve runs until killed; its lifetime is uptime, not latency.
    else if (route === 'serve' && !rest.includes('--rotate-token'))
      usageCmd += ' --attached';
