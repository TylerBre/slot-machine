...
export async function run(argv) {
  ...
  process.on('exit', (code) => {
    recordUsage({
      ...
      // must not read a quiet fleet as a broken command.
      ok: code === 0 || (code === 3 && (usageCmd ?? '').startsWith('watch')),
      ms: Date.now() - t0,
      repo: REPO_NAME,
  ...
  const main = async () => {
    ...
    if (isWatch)
      ...
      usageCmd += ' --wait';
    // sm watch: bucket the blocking mode apart from one-shot checks - its runtime is idle
    // waiting, not command latency, and would otherwise poison avg/max in sm stats.
    else if (route === 'watch')
      usageCmd += rest.includes('--check') ? ' --check' : ' --loop';
