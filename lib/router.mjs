...
import { REPO_DIR, REPO_NAME } from './constants.mjs';
import { helpFor } from './help.mjs';
import { CONFIG_ERROR } from './context.mjs';
import { recordUsage } from './usage.mjs';
import { die } from './format.mjs';
import { cmdRepoInspect, cmdRepoLs, cmdRepoRm, cmdUse } from './commands/repo.mjs';
import { cmdAttach, cmdBuild, cmdDetach, cmdKill, cmdLs, cmdReload } from './commands/session.mjs';
import { cmdAdd, cmdFocus, cmdFree, cmdInfo, cmdReset, cmdRm } from './commands/slot.mjs';
import { cmdDispatch, cmdLogs, cmdPreflight, cmdPs, cmdRole, cmdWorkerKill } from './commands/worker.mjs';
...
import { cmdDoctor, cmdStats } from './commands/top.mjs';
...
// (help / --help / -h / bare namespace) returns before dispatch, so only doctor lists here.
const CONFIG_TOLERANT = new Set(['doctor']);
...
export async function run(argv) {
  ...
  const main = async () => {
    ...
    if (ri >= 0) {
      ...
      const eq = args.findIndex(arg => arg.startsWith('--repo='));
      if (eq >= 0)
        args.splice(eq, 1);
    }
    ...
    if (rest.includes('--watch') || follow)
      usageCmd += ' --watch';
    else if (rest.includes('--wait'))
      usageCmd += ' --wait';
