...
/**
 * The JSON Schema the web (sm serve) surface uses as a command's inputSchema: identical
 * to mcpInputSchema but dropping args marked `webHidden` in x-cli - the web twin of
 * mcpHidden. Applied to consumption verbs (msg-inbox --clear/--unread eat shared state)
 * and blocking display flags (--watch/--follow/--hook/--loop hold a terminal, which a
 * POST must never do).
 * @param {object} spec - a command arg-spec.
 * @returns {object} the web inputSchema.
 */
export function webInputSchema(spec) {
  const { 'x-mcp': _mcp, 'x-web': _web, 'x-exit': _exit, 'x-cli': cli, $schema: _schema, $id: _id, title: _title, description: _description, ...rest } = spec;
  const hidden = new Set(Object.entries(cli.args).filter(([, entry]) => entry.webHidden).map(([name]) => name));
  const properties = Object.fromEntries(Object.entries(rest.properties ?? {}).filter(([name]) => !hidden.has(name)));
  const schema = { ...rest, properties };
  if (rest.required)
    schema.required = rest.required.filter(name => !hidden.has(name));
...
/**
 * Map a command's exit code to the versioned envelope's outcome. Exit 0 is 'ok'; a code
 * named in the spec's optional x-exit map is a DEFINED result (still ok: true - e.g.
 * watch's exit 3 = 'nothing-to-report', matching the router's own reading); any other
 * non-zero is an error.
 * @param {object} spec - a command arg-spec.
 * @param {number} exitCode - the child's exit code.
 * @returns {{ok: boolean, outcome: string}} the envelope outcome.
 */
export function commandOutcome(spec, exitCode) {
  if (exitCode === 0)
    return { ok: true, outcome: 'ok' };
  const named = spec['x-exit']?.[String(exitCode)];
  return named ? { ok: true, outcome: named } : { ok: false, outcome: 'error' };
}
