...
export async function cmdPs(argv) {
  ...
  const snapshot = () => {
    ...
    return labels.map((lbl) => {
      ...
      const activity
        = worker === 'live'
          ? activityOf(REPO_DIR, lbl, cap?.ok ? cap.value : '', !!pane)
          : '-';
...
export async function cmdLogs(argv) {
  ...
  const snapshot = () => {
    ...
    const capRes = pane ? mux('capture', { paneId: pane }) : null;
    const capture = capRes?.ok ? capRes.value : '';
...
export async function cmdWorkerKill(argv) {
  ...
  const killCap = mux('capture', { paneId: pane });
  const activity = activityOf(REPO_DIR, label, killCap.ok ? killCap.value : '', true);
  ...
  const pidRes = mux('panePid', { paneId: pane });
  if (!pidRes.ok)
    die(`kill: could not resolve the pane process for slot ${label}${pidRes.err === 'unsupported' ? ` (not supported by the ${pidRes.detail.split(' ')[0]} backend)` : ''}`);
  const panePid = String(pidRes.value);
