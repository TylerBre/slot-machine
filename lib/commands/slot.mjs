...
export async function resetSlot(name, { force = false, hardWorker = false } = {}) {
  ...
  if (dirty) {
    ...
    // The dirt-rule's physical half: sm's own artifacts are excluded from every destructive
    // git operation - without -e, clean deletes the untracked worktree document (and with it
    // the worker identity that must survive a reset). The tmp pattern also covers broken husks.
  ...
  if (hardWorker) {
    // Record-before-mutation (this is sm-owned state with a v1 record type): the fact that the
    // conversation was replaced is journaled durably BEFORE the worker section is cleared.
    // Journal failure here degrades (history is an aid, not a ledger) - the reset proceeds.
...
/**
 * Landed-work proof: is everything committed in this slot actually preserved somewhere that
 * survives a hard reset? True when (a) the slot's own remote branch contains HEAD (nothing
 * unpushed), or (b) a merged PR's head commit contains HEAD (covers the common case where the
 * remote branch was auto-deleted on merge). Guards the dispatcher's automatic force-reset of
 * "merged" slots against stragglers - commits made after the PR merged, which a reset would
 * silently destroy.
