...
import { startServe } from '../serve/server.mjs';
import { argOptions, parseCmd } from './shared.mjs';

// The spawn target must be version-stable across upgrades: $SM_SERVE_BIN (dev escape hatch)
// else `sm` on PATH, whose shims repoint atomically on upgrade. NEVER an import.meta.url
// sibling - a Homebrew Cellar realpath goes stale under a live serve the moment brew upgrades.
...
export async function cmdServe(argv) {
  ...
  // One-time pairing print. sm serves no HTML - the cockpit runs on its own origin and
  // proxies /api here, so pairing happens THERE. The fragment never reaches a server or a
  // log, but it DOES land in browser history - which is why rotation is cheap and surfaced.
