...
export async function cmdServe(argv) {
  ...
  const running = await startServe({ port, uiDir: values.ui ?? null, spawnTarget, repos });
  console.log(`sm serve on http://127.0.0.1:${running.port} (repos: ${Object.keys(repos).join(', ')})`);
  // The one-time pairing print: the fragment never reaches a server or a log, but it
  // DOES land in browser history - which is why rotation is cheap and surfaced.
  console.log(`pair: http://127.0.0.1:${running.port}/#token=${running.token}`);
  const stop = async () => {
    await running.close();
    process.exit(0);
  };
