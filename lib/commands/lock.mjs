...
export async function cmdClaim(argv) {
  ...
  // No pid recorded here: the resource's real backing process (e.g. the shared browser) is
  // launched AFTER the claim and resolved live at release (resourceProcessPids); the holder
  // is identified by cwd. The tmux pane pid we used to store was a misleading handle.
  ...
  for (;;) {
    ...
    if (!result.ok && result.holder) {
      // Steal when the holder's worker is demonstrably dead, or on --force; tiny release->claim race
      // is fine. 'none' (no pane info at all) is NOT auto-stolen - that needs an explicit --force.
      const hl = labelFromDir(result.holder.cwd);
      const holderDead = hl ? slotWorkerMap().get(hl) === 'dead' : false;
      if (holderDead || values.force) {
        // Stealing is a release + re-claim: like cmdRelease it must terminate the resource's
        // backing process (e.g. the orphaned browser), not just drop the lockfile.
...
export function cmdRelease(argv) {
  ...
  // the lockfile (e.g. the browser resource -> the shared Chromium, which single-locks its
  // profile and would otherwise block the next claimant). The resource->process mapping is a
  // generic resolver seam (see resourceProcessPids); slots have no such process.
