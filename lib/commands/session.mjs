...
import { loadRoster, safeLaunchLine } from '../agents/index.mjs';
import { argOptions, parseCmd, resolveSession } from './shared.mjs';
...
export async function cmdBuild(argv) {
  ...
  for (let index = 0; index < slots.length; index += perN) {
    ...
    const line0 = safeLaunchLine(REPO_DIR, group[0].slice(PREFIX.length), dir0);
    if (line0)
      sendLine(pane, line0);
    for (let paneIndex = 1; paneIndex < group.length; paneIndex++) {
      ...
      const lineN = safeLaunchLine(REPO_DIR, group[paneIndex].slice(PREFIX.length), dirN);
      if (lineN)
        sendLine(pane, lineN);
    }
