...
import { listResourceLocks, readLock } from '../slots/locks.mjs';
 ...
 * (tmux + lockfiles only, no git/gh). `sm slot ls` stays the reusability authority.
...
export async function cmdFloor(argv) {
  ...
  const slots = labels.map((lbl) => {
    ...
    const activity = worker === 'live' ? activityOf(REPO_DIR, lbl, capture, !!pane) : '-';
  ...
  const snapshot = {
    ...
    inbox: { unread: readInbox(REPO_NAME).length },
  };
  if (values.json) {
    emitJson(snapshot);
    return;
  }
  const sess = snapshot.sessions.map(row => `${row.name} (${row.slots} slots${row.attached ? ', attached' : ''})`).join('  ');
  console.log(`${clr.bold(REPO_NAME)}  ${sess || clr.dim('no running session')}`);
  const lw = Math.max(4, ...slots.map(row => row.slot.length));
  for (const row of slots) {
    const worker = (row.worker === 'live' ? clr.green : row.worker === 'dead' ? clr.red : clr.dim)(row.worker.padEnd(5));
    const lock = row.locked ? clr.red('lock') : clr.dim('-   ');
    console.log(`  ${clr.bold(row.slot.padEnd(lw))} ${worker} ${row.activity.padEnd(8)} ${lock} ${row.task ? oneLine(row.task, 60) : clr.dim('-')}`);
  }
  for (const res of snapshot.resources)
    console.log(`  ${clr.bold(res.resource)}  held${res.task ? `  ${res.task}` : ''}`);
  console.log(clr.dim(`  inbox: ${snapshot.inbox.unread} unread`));
}
