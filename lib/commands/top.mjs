...
export async function cmdDoctor(argv) {
  ...
  for (const name of inUseInstances(rosterCfg, REPO_DIR)) {
    ...
    const doctorArgs = { env: inst.env, mcpServers, deskDir: DOCS };
    ...
    if (delivery && fix && !(delivery.stop && delivery.promptSubmit)) {
      const setup = callOp(inst.plugin, 'deliverySetup', { deskDir: DOCS, env: inst.env });
      if (setup.ok) {
    ...
    if (delivery) {
      ...
      add(
        ...
        'ok',
        wired
          ? `watch hooks wired in ${DOCS}/.claude/settings.json (active only under SM_DESK=1)`
