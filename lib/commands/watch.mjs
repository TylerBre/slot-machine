...
export async function runHook({ type, world = null, now = null }) {
  ...
  // but only the session launched with SM_DESK=1 is the delivery seat. Env inheritance into
  // hook subprocesses is undocumented, so the gate fails CLOSED - no ack, no output, exit 0.
