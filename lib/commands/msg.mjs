...
export function cmdReport(argv) {
  ...
  // Verb adoption nudge, at the moment of writing, every time - stderr only, so output
  // and exit code are untouched. Verbs are what let the watch triage without reading prose.
...
// CLAIM it - conditionally, inside the serialized document mutation, so a racing
// `worker run`/`msg -f` (CLI, MCP, or a serve tab) can never double-book the same slot:
// the loser's claim is refused and it re-picks from the remaining gathered candidates.
