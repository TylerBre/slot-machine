...
import { claimIfFree, readLock, writeLock } from '../slots/locks.mjs';
...
export function cmdReport(argv) {
  ...
  // Verb nudge on stderr only (output and exit code untouched): verbs are what let the
  // watch triage without reading prose.
  if (parseVerb(positionals[0]) == null)
    console.error('tip: prefix with done:/blocked:/needs-decision:/failed:/working:/paused: so the dispatcher\'s watch can triage it');
  if (values.json) {
...
// Severity coloring for the human render: attention verbs red, done green, progress dim.
const VERB_COLOR = {
  'done': clr.green,
  'blocked': clr.red,
  'needs-decision': clr.red,
  'failed': clr.red,
  'working': clr.dim,
  'paused': clr.yellow,
};
...
function printInbox(entries, json) {
  // The verb is computed here, at display time - never persisted (schema stays unbumped).
  const tagged = entries.map(entry => ({ ...entry, verb: parseVerb(entry.message) }));
  if (json) {
    emitJson(tagged);
    return;
  }
  ...
  for (const entry of tagged) {
    ...
    const tag = entry.verb ? `${VERB_COLOR[entry.verb](`[${entry.verb}]`)} ` : '';
    console.log(`${clr.bold(entry.slot || '-')}  ${tag}${clr.dim(age)}  ${entry.message}`);
  }
}

// Send one submitted line to the LIVE slot panes matching `want` (null = all). A non-live pane
// (agent exited to a shell) is NEVER typed into - that would run the task text as a shell
// command; it is marked .dead instead. Live targets get a verified-submit .submitted flag.
...
// --until-idle: a target that typed but never submitted still has its text in the composer.
// Re-submit those on an interval - never re-type, which would duplicate the pending text -
// until every composer accepts or the timeout. Mutates each target's .submitted in place.
...
// Pick the first reusable slot with a live worker (resetting a merged one), then CLAIM it
// conditionally inside the serialized document mutation, so racing dispatchers can never
// double-book a slot: the loser's claim is refused and it re-picks from the remaining candidates.
async function pickReusable(json, who, session, msg) {
  const rows = await freenessRows(json, 'finding a free slot...');
  let reset = false;
  const picked = await selectAndClaim(rows, async (pick) => {
    reset = false;
    if (pick.status === 'merged') {
      // Landed-work proof before the destructive force-reset: "merged" means every PR is merged,
      // NOT that every local commit landed - a straggler committed after the merge would be
      // silently destroyed. Refuse and point at the slot instead.
      ...
      try {
        // force: squash-merged commits are not ancestors of origin/base, so reset's
        // unmerged-commit guard would refuse a slot whose work already landed.
    ...
    return claimIfFree(join(DOCS, PREFIX + pick.slot), { session, task: oneLine(msg, 140) });
  });
  if (!picked)
    die(`${who}: no free slot with a live worker (see: sm slot ls)`);
  return { slot: picked.slot, from: picked.status, reset };
}

/**
 * The --brief worker preamble: model onboarding + the report-verb contract, carried to
 * the worker WITH the task - the contract travels in the dispatch itself.
 * @param {string} slot - Target slot label.
 * @param {string} msg - The task text.
 * @returns {string} the full message to type into the worker pane.
 */
export function briefText(slot, msg) {
  return `You are the worker for slot ${slot} in the slot-machine model - run 'sm worker role' for how to operate here, lead every 'sm msg report' with a verb (done:/blocked:/needs-decision:/failed:/working:/paused:), then do this task: ${msg}`;
}
...
export async function cmdMsg(argv, who = 'send') {
  ...
  const text = values.brief && want && want.size === 1 ? briefText([...want][0], msg) : msg;
  ...
  if (!msg.startsWith('/')) {
    ...
    for (const target of submitted) {
      ...
      // Fresh claim when unheld/dead; a TARGETED re-dispatch (-s/-f) to a live-held slot
      // refreshes the recorded task; a broadcast (want == null) leaves an existing lock's
      // task label untouched.
      if (!held || want != null)
      ...
      // reports - the journal records what verifiably happened, never what is about to be.
      try {
