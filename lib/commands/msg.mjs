...
import { appendReport, clearInbox, consumeReports, readInbox, shapeInbox, waitForReports } from '../inbox.mjs';
import { argOptions, freenessRows, parseCmd, resolveSession } from './shared.mjs';
import { resetSlot } from './slot.mjs';
...
export async function cmdInbox(argv) {
  ...
  if (values.watch) {
    ...
    const baseline = readInbox(REPO_NAME).length;
    const fresh = await waitForReports(REPO_NAME, { baseline });
    if (fresh.length) {
      printInbox(fresh, values.json);
      // Consume only the reports we displayed - not the whole inbox, which would drop reports that
      // predate the watch or arrived during the print without ever being shown.
      if (values.clear)
        consumeReports(REPO_NAME, baseline, baseline + fresh.length);
      return;
    }
    if (values.json)
      emitJson([]);
...
function sendToPanes(session, want, msg) {
  ...
  // Deliver to a Claude composer with verified submit (not just typed); each target gets a
  // .submitted flag so the caller can report delivery honestly.
  for (const target of targets) target.submitted = sendMessage(target.pid, msg);
  return targets;
}
...
async function retryUntilIdle(targets) {
  ...
  while (pending.length && Date.now() < deadline) {
    ...
    for (const target of pending) target.submitted = resubmitMessage(target.pid);
...
export async function cmdMsg(argv, who = 'send') {
  ...
  if (!msg.startsWith('/')) {
    ...
    for (const target of targets.filter(entry => entry.submitted)) {
      const tdir = join(DOCS, PREFIX + target.lbl);
      if (!(readLock(tdir) && liveWorkers.get(target.lbl) === 'live'))
        writeLock(tdir, { session: sess, pane: target.pid, task: oneLine(msg, 140) });
    }
  }
  const sent = targets.filter(target => target.submitted).map(target => target.lbl);
  const unsubmitted = targets.filter(target => !target.submitted).map(target => target.lbl);
  ...
  console.log(
    `delivered to ${sent.length}/${targets.length} slot pane(s) in '${sess}'${picked ? ` (first-free: ${clr.bold(picked.slot)}${picked.reset ? ', reset from merged' : ''})` : ''}`,
  );
}
