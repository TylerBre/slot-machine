...
function sendToPanes(session, want, msg, liveWorkers) {
  ...
  for (const target of targets) {
    ...
    target.submitted = sendMessage(target.pid, msg);
  }
  return targets;
}
...
export async function cmdMsg(argv, who = 'send') {
  ...
  // messages like /clear, not task dispatch).
  if (!msg.startsWith('/')) {
  ...
  if (dead.length) {
    console.error(
      `${who}: warning - worker not live (Claude exited to a shell); skipped, NOT delivered: ${dead.join(' ')}`,
    );
  }
