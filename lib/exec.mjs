...
// ANSI (-e) and strips faint ghost-text first, so a dim autocomplete suggestion after the
// prompt never reads as unsent input. No prompt (agent already working, or a shell pane)
// counts as empty so we never falsely block.
function composerEmpty(pane) {
  const cap = tmuxOut(['capture-pane', '-p', '-e', '-t', pane]);
  if (cap == null)
    return true;
  const lines = stripGhostText(cap).split('\n');
