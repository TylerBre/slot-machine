...
export async function prMap(slug) {
  return (await prMapChecked(slug)).map;
}

/**
 * Parse `gh pr list` TSV lines into the branch -> PRs map. Pure.
 * @param {string} out - TSV output (headRefName, number, state, headOid per line).
 * @returns {Map<string, Array<{number: number, state: string, headOid: string|null}>>} branch -> PRs map.
 */
export function parsePrTsv(out) {
  const map = new Map();
...
/**
 * prMap with failure DISTINGUISHABLE from empty: the watch path must never read a gh
 * outage as "no PRs" (run() swallows failures to ''; a supervision predicate fed that
 * would silently drop pr-merged events). ok:false still carries whatever partial
 * output gh produced, matching prMap's legacy best-effort contract.
 * @param {string} slug - owner/name repo slug.
 * @returns {Promise<{ok: boolean, map: Map<string, Array<{number: number, state: string, headOid: string|null}>>}>} checked map.
 */
export async function prMapChecked(slug) {
  const args = [
    'pr',
    'list',
    '--repo',
  ...
  try {
    const out = (await pexec('gh', args, { encoding: 'utf8' })).stdout || '';
    return { ok: true, map: parsePrTsv(out) };
  }
  catch (err) {
    return { ok: false, map: parsePrTsv(err.stdout || '') };
  }
}
