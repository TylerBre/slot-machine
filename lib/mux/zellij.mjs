...
// surface it is built on. Groups are zellij tabs; pane labels are zellij pane names, which
// render on the pane frame natively. Known limits vs tmux: see README.md.
...
export default {
  ...
  spawnPane({ group, cwd, label }) {
    // No -d direction: zellij (0.44.x) silently drops the pane when a direction is combined
    // with --tab-id. Zellij tiles the tab on its own.
