...
// Pane labels live in the sm-owned @smslot option, never @cclabel (see README.md).
...
// sm-owned pane option carrying the slot label - deliberately NOT @cclabel, a display slot
// users' own tooling writes; correlation must never read user display text as a slot label.
const LABEL_OPT = '@smslot';
...
export default {
  ...
  streamStart({ paneId, sink, byteCap }) {
    // pipe-pane executes its argument via a shell - the ONE sanctioned shell string in this
    // codebase. Nothing request-derived may enter it; validated HERE regardless of caller.
    ...
    // -o: only open when NO pipe exists - never clobber a pipe someone else owns.
