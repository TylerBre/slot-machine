...
// backend is a config setting (settings.mux), defaulting to tmux; backends are built-in only
// (the multiplexer universe is small and closed - user mux plugins can come when a third
// multiplexer actually exists). All ops dispatch through the shared plugin contract's guarded
// call path, so a backend gap surfaces as an UNSUPPORTED envelope, never a crash.
...
/**
 * Drop faint/dim "ghost text" from ANSI-captured pane content, then strip every remaining
 * escape sequence. Claude's composer renders autocomplete suggestions dim (SGR 2) after the
 * prompt; a plain-text capture cannot tell them from real typed input, so a composer scan
 * would read a ghost suggestion as unsent text and falsely report a failed submit. Pure,
 * exported for tests.
...
// captured pane - content after the prompt means text is still sitting unsent. Captures with
// ANSI and strips faint ghost-text first, so a dim autocomplete suggestion after the prompt
// never reads as unsent input. No prompt (agent already working, or a shell pane) counts as
// empty so we never falsely block.
