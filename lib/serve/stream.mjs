...
export function createStreamHub({ spawnTarget, repos, intervals = {}, mirror = null }) {
  ...
  function pullSpool(conn, slot, state) {
    ...
    if (size < state.offset) {
      // truncated underneath (a rotate landed): the manager's `rotated` event queues the
      // reset for every viewer; rewinding silently would replay bytes into an uncleared
      // terminal, so the reset ALWAYS precedes re-reads from 0.
      state.offset = 0;
    ...
    if (size > mirrorRotateBytes) {
      ...
      // manager announces `rotated` to every viewer (reset + reseed) when it happens
