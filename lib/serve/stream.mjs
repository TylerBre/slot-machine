...
export function createStreamHub({ spawnTarget, repos, intervals = {}, mirror = null }) {
  ...
  function pullSpool(conn, slot, state) {
    ...
    if (size < state.offset)
      state.offset = 0; // truncated underneath: the rotate path queued the reset
    if (size > mirrorRotateBytes) {
      state.offset = 0;
      state.queue.length = 0;
      state.queue.push({ channel: `mirror:${slot}`, t: 'reset' });
      // fire-and-forget: the manager stops, truncates, restarts the pipe
      mirror.rotate(conn.repoName, slot).catch(() => {});
      pump(conn);
      return;
    }
  ...
  async function attachMirror(conn, slot) {
    ...
    const state = { queue: [], offset: 0, sink: null, watcher: null, opened: false };
    ...
    if (conn.closed) {
      mirror.closeMirror(conn.repoName, slot);
      return;
    }
    state.sink = info.sink;
    state.opened = true;
    state.queue.push({ channel, t: 'open', cols: info.cols, rows: info.rows, mode: info.mode });
    const onLost = ({ repo, slot: lostSlot }) => {
      if (repo === conn.repoName && lostSlot === slot) {
  ...
  function handle(req, res, repoName, url) {
    ...
    const mirrorSlots = requestedRaw
