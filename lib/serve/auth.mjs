// sm serve auth: the pairing token and stateless HMAC cookie sessions.
//
// The honest frame (from the design review): the pairing token is an arbitrary-code-
// execution credential for this host - the surface it guards dispatches tasks to agents
// holding shell, git, and gh. Every choice here is sized against that: 32 random bytes,
// 0600 in a 0700 dir, constant-time compares everywhere, and sessions that are HMAC
// derivatives of the token (stateless - a restarted serve honors cookies minted by a
// dead one, invariant-1 style) so no JS-readable credential ever exists after pairing.
...
/**
 * The serve state home (token, and later the mirror registry/spools). Overridable for
 * tests via SLOT_SERVE_DIR, mirroring the inbox/journal seams.
