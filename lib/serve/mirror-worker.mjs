...
function handle(message) {
  ...
  if (op === 'seed') {
    // One full-screen capture appended as a redraw frame: pipe mode only carries
    // FORWARD bytes, so without this an idle pane mirrors as blackness until it next
    // prints - the viewer wants the current screen immediately.
