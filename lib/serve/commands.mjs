...
// exit in the versioned envelope. See README.md.
...
// Reads ride through version skew: a stale serve may OBSERVE the fleet but must not
// MUTATE through a newer binary its loaded schemas cannot describe. Unlisted = mutating.
...
export function createCommandRunner({ spawnTarget, repos, isSkewed, spawnTimeoutMs = SERVE_SPAWN_TIMEOUT_MS }) {
  ...
  async function run(tool, body) {
    ...
    // The repo pin: exact registered-name match, resolved serve-side, refused BEFORE any
    // spawn. resolveActive would take an arbitrary path, so this gate is the only backstop.
    ...
    try {
      ...
      if (tool === 'worker-run') {
        ...
        // free-slot pool; serialized, the second sees the first's claim instead of losing.
