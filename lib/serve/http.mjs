 ...
 */
export async function startServe({ port = SERVE_PORT, uiDir = null, spawnTarget, repos = {}, spawnTimeoutMs = undefined, streamIntervals = undefined } = {}) {
  ...
  const hub = createStreamHub({ spawnTarget, repos, intervals: streamIntervals });
  ...
  return {
    ...
    close: () => new Promise((resolvePromise) => {
      ...
      // their cursor against the next serve), then drop sockets, then close, then
      // release the instance pidfile.
      hub.closeAll();
