...
// refuses (defense in depth, any auth kind); the cockpit's proxy forwards the browser's
// same-origin value verbatim. Absent the header entirely, only bearer scripting (curl)
// passes - sm hosts no pages at its own origin, so there is no Origin value a
// cookie-authed browser request could legitimately present. (An Origin-vs-Host
// comparison lived here in the static-hosting era; API-only made it dead code.)
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (!path.startsWith('/api/')) {
        // sm serve is API-only, by design: sm never serves HTML and holds no opinion
        // about a UI's content policy. The cockpit runs on its OWN origin and proxies
        // /api here (same-origin for the browser, so the cookie model is untouched).
