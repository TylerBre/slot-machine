...
Top level
  ...
  serve                   API bridge for the dispatcher cockpit (API-only, localhost, long-running)
...
Examples
  ...
  'serve': `sm serve [--port N] [--rotate-token]

Run the API bridge the dispatcher cockpit (sm-cockpit) talks to. API-ONLY by design:
sm serves no HTML - the cockpit runs on its own origin and proxies /api here (that
keeps the browser same-origin, so the cookie model needs no CORS). Binds 127.0.0.1
ONLY; every request needs a paired session (or a Bearer token for curl). Long-running.
