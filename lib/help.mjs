...
Top level
  ...
  floor                   one-shot fleet snapshot: sessions, slots, locks, inbox, watch
  journal                 the repo's turn journal: what happened, when, to which worker
  watch                   supervision digest: --check peeks, --check --ack surfaces durably
  serve                   HTTP+SSE bridge for the dispatcher cockpit (localhost, long-running)
...
Examples
  ...
  'watch': `sm watch [--check [--ack]] [--loop] [--timeout SECS] [--json]

Supervision: gather fleet evidence, classify every event as absorb (positive evidence of
work - noted, not shown) or surface (needs the dispatcher), and print a digest. Surfaced:
reports with done:/blocked:/needs-decision:/failed: or no verb, stale paused: reports
(> 60m), stalled working: reports (> 30m with no live activity), crashed workers (claim
held, worker gone in two samples), merged PRs on claimed slots.

Modes
  --check          one-shot peek: print the digest, change NOTHING. Safe from any seat,
                   any time. Exit 0 = digest, 3 = nothing to report.
  --check --ack    durably surface: journal the facts, advance the surfaced watermark for
                   emitted events (capped at 5 per digest; overflow drains on later acks).
                   The FIRST ack baselines past the existing backlog and says so.
  (bare) [--loop]  blocking human watch: wakes on new reports (push) and re-checks on a
                   15s cadence for report-less events (crash). Arms the floor's watch
                   marker; acks what it prints. --timeout bounds the wait (exit 3).

The watch never marks anything READ - reading is the dispatcher's explicit act via
sm msg inbox --unread (a separate cursor from the surfaced watermark).

Examples
  sm watch --check            # anything need me? (peek)
  sm watch --check --ack --json
  sm watch --timeout 600      # block up to 10 minutes for the next event`,

  'serve': `sm serve [--port N] [--ui DIR] [--rotate-token]

Run the HTTP+SSE bridge the dispatcher cockpit (sm-cockpit) talks to. Binds 127.0.0.1
ONLY; every request needs a session paired from the printed one-time URL (or a Bearer
token for curl). Long-running - Ctrl-C to stop. Exposes exactly the x-web command
allowlist plus the event stream; nothing else.

Options
  --port N         bind port (default 7767)
  --ui DIR         also host a built cockpit UI from DIR (strict CSP, SPA fallback)
  --rotate-token   re-mint the pairing token (kills existing sessions) and exit

Examples
  sm serve --ui ~/Documents/sm-cockpit/dist
  sm serve --rotate-token`,
