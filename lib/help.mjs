...
one row per slot (worker live/dead/none, activity, lock + task), held resource locks, and
the unread inbox count, in a single command instead of four (session ls + worker ps +
lock ls + msg inbox). Cheap: multiplexer + lockfiles only, no git/gh - 'sm slot ls' stays
the authority on which slots are reusable.
