 ...
 * @param {string} text - The existing tmux.conf contents.
 * @returns {string} The contents with the marked block upserted.
 */
export function upsertBlock(text) {
  const re = new RegExp(`${escapeRe(BEGIN)}[\\s\\S]*?${escapeRe(END)}`);
  if (re.test(text))
    return text.replace(re, TMUX_BLOCK);
  return `${(text.trimEnd() ? `${text.trimEnd()}\n\n` : '') + TMUX_BLOCK}\n`;
}

/**
 * Prefer an existing ~/.tmux.conf, then the XDG path; default to ~/.tmux.conf.
 * @returns {string} The resolved tmux.conf path.
 */
 ...
 * @returns {{ path: string, changed: boolean }} The conf path and whether it was written.
 */
export function writeTmuxBlock() {
  const confPath = tmuxConfPath();
  const cur = existsSync(confPath) ? readFileSync(confPath, 'utf8') : '';
  const next = upsertBlock(cur);
  const changed = next !== cur;
  if (changed)
    writeFileSync(confPath, next);
  return { path: confPath, changed };
}
 ...
 * @param {string} [liveBorderStatus] - The live server's pane-border-status, if running.
 * @returns {{ ok: boolean, source: string, value: string }} The status verdict.
 */
export function tmuxTitlesStatus(liveBorderStatus) {
  ...
  const confPath = tmuxConfPath();
  const text = existsSync(confPath) ? readFileSync(confPath, 'utf8') : '';
