...
/**
 * The slot label a pane maps to, or null when it is not a slot pane. Label-first: a label
 * stamped at spawn (the mux backend's pane label) is authoritative and survives the worker
 * changing directory; panes from sessions built before labels existed fall back to deriving
 * the label from the pane's directory - counted only when it is exactly the slot root.
 * @param {object} pane - a Pane record (see lib/mux/contract.mjs).
 * @param {string} root - the docs root directory.
 * @param {string} prefix - the slot worktree name prefix.
 * @returns {string|null} the slot label, or null.
 */
export function paneLabel(pane, root, prefix) {
  if (pane.label)
    return pane.label;
...
/**
 * Given Pane records (from the mux backend's listPanes), pick the slot panes under `root`,
 * optionally filtered to `want` (a Set of labels, or null for all). Returns [{ id, lbl }]
 * in input order.
 * @param {object[]} panes - Pane records (see lib/mux/contract.mjs).
 * @param {string} root - the docs root directory.
 * @param {string} prefix - the slot worktree name prefix.
 * @param {Set<string>|null} want - labels to keep, or null for all.
 * @returns {Array<{id: string, lbl: string}>} the matching slot panes in input order.
 */
export function selectPanes(panes, root, prefix, want) {
  const out = [];
  for (const pane of panes) {
    if (!pane)
      continue;
    const lbl = paneLabel(pane, root, prefix);
    if (lbl == null)
      continue;
    if (want && !want.has(lbl))
      continue;
    out.push({ id: pane.id, lbl });
  }
  return out;
}
...
/**
 * Pick-and-claim over a candidate pool: pickDispatchSlot chooses, tryClaim attempts the
 * conditional claim, and a REFUSED claim (a racing dispatcher won that slot) drops the
 * candidate and re-picks from the rest - never a re-gather, never a double-book. Pure
 * control flow; the caller owns the IO inside tryClaim.
 * @param {object[]} rows - the classified slot rows (as pickDispatchSlot expects).
 * @param {(pick: object) => Promise<boolean>} tryClaim - attempt the claim; true = won.
 * @returns {Promise<object|null>} the claimed row, or null when candidates exhaust.
 */
export async function selectAndClaim(rows, tryClaim) {
  let pool = rows;
  for (;;) {
    const pick = pickDispatchSlot(pool);
    if (!pick)
      return null;
    if (await tryClaim(pick))
      return pick;
    pool = pool.filter(row => row.slot !== pick.slot);
  }
}
