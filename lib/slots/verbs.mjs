// Report verbs: the worker->dispatcher triage vocabulary. A report leads with "<verb>:"
// so supervision can tell "needs me" from "just telling me". Verbs are parsed at READ
// time and never persisted - the inbox schema stays unbumped, so a version-skewed
// reader can never drop new reports as malformed.
...
/**
 * The absorb/surface policy - PURE: the gatherer does IO, this decides. Same evidence in,
 * same classification out; the only cross-run memory is the journal facts and the
 * surfaced watermark passed IN.
 *
