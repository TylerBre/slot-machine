...
import { activeOverride, classifySlot, issueFromText, selectPanes } from './pure.mjs';
  ...
  const rows = await Promise.all(
    slots.map(async (name) => {
      ...
      // Backstop: reusable-looking + a live worker mid-task (no lock yet) => not free; don't dispatch over it.
        ...
        const activity = activityOf(REPO_DIR, short, pane ? (tmuxOut(['capture-pane', '-p', '-t', pane]) ?? '') : '', !!pane);
