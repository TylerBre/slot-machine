...
// A pane's worker is live while something beyond a bare shell runs in it: not exited and the
// foreground command is not a shell (Claude reports its version string as the command).
...
/**
 * Pure core of the watch-side worker sample: panes -> plain {slot: 'live'|'dead'} object
 * (classify's evidence shape; a claimed slot ABSENT from the sample reads as 'none' there).
 * @param {object[]} panes - pane records from the mux backend.
 * @returns {object} slot label -> 'live' | 'dead'.
 */
export function workersFromPanes(panes) {
  const workers = {};
  for (const pane of panes) {
    const lbl = paneLabel(pane, DOCS, PREFIX);
    if (lbl == null)
      continue;
    workers[lbl] = workers[lbl] === 'live' || paneLive(pane) ? 'live' : 'dead';
  }
  return workers;
}

/**
 * Watch-side worker sample with the mux envelope's ok PROPAGATED: a failed backend reads
 * as {ok: false}, never as "every worker vanished" - the []-coercion in allPanes() is
 * exactly what would fabricate N simultaneous crash alarms from one transient mux error.
 * @returns {{ok: boolean, workers: object}} envelope-checked sample.
 */
export function slotWorkerSample() {
  const res = mux('listPanes', { scope: 'all' });
  return res.ok ? { ok: true, workers: workersFromPanes(res.value) } : { ok: false, workers: {} };
}
