...
const LOCK_ELEVATORS = [
  ...
  // Timestamps are normalized to epoch ms: pre-v1 relics written by old workers carry ISO
  // strings, which would otherwise poison age math all the way up the stack.
  (lock, cwd) => ({
    v: 2,
    cwd: lock.cwd ?? cwd ?? null,
    ts: epochMs(lock.ts),
 ...
 * one serialized mutateDoc mutation, so two concurrent dispatchers (CLI, MCP, serve tabs)
 * can never both claim one slot. Closes the select-then-claim TOCTOU: the loser gets
 * false and re-picks instead of silently double-booking. A refused claim leaves the held
 * claim untouched (the document is rewritten atomically with identical claim content -
 * only the envelope ts moves).
 * @param {string} dir - the slot worktree directory.
