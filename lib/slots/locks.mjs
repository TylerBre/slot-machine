...
// claim (the lock), worker (the bound conversation), turn (an in-flight session turn).
// readLock keeps its historical contract (flat claim, or null when unclaimed). See README.md.
import { closeSync, existsSync, fsyncSync, ftruncateSync, openSync, readdirSync, readFileSync, renameSync, statSync, unlinkSync, writeSync } from 'node:fs';
// (writeFileSync is deliberately absent: every document write flows through mutateDoc - there is no unserialized write path.)
...
// A timestamp as epoch ms: ISO strings from pre-v1 relic locks parse; unparseable becomes 0
// ("long ago" - honest for a file that old).
const epochMs = ts => (typeof ts === 'string' ? Date.parse(ts) || 0 : ts);
...
const LOCK_ELEVATORS = [
  ...
  // Timestamps normalize to epoch ms (pre-v1 relics carry ISO strings that would poison age math).
  (lock, cwd) => ({
    v: 2,
    cwd: lock.cwd ?? cwd ?? null,
    ts: epochMs(lock.ts),
...
/**
 * This process's own pid-identity token (for durable records other processes will
 * liveness-check with pidIdentityLive). Null when ps is unavailable.
 * @returns {string|null} the start-time token.
 */
export function ownPidStart() {
  return lstartOf(process.pid);
}
...
/**
 * Claim a slot ONLY if it is currently unclaimed - the whole check-then-claim runs inside
 * one serialized mutateDoc mutation, closing the select-then-claim TOCTOU: two concurrent
 * dispatchers can never both claim one slot; the loser gets false and re-picks. A refused
 * claim leaves the held claim untouched (only the envelope ts moves).
 * @param {string} dir - the slot worktree directory.
 * @param {object} options - the claim fields (as writeLock).
 * @param {string|null} options.session - the multiplexer session name.
 * @param {string|null} options.task - the task text.
 * @param {string|null} options.issue - the issue id (defaults to one parsed from the task).
 * @returns {boolean} true when this call won the claim.
 */
export function claimIfFree(dir, { session = null, task = null, issue = null } = {}) {
  let claimed = false;
  mutateDoc(dir, (current) => {
    if (current.claim != null)
      return current;
    claimed = true;
    return { ...current, claim: { session, task, issue: issue ?? issueFromText(task), ts: Date.now() } };
  });
  return claimed;
}
...
/**
 * Force-clear the turn section regardless of holder - the destructive verbs' --force path.
 * Goes through the serialized write protocol like every mutation; never a blind file operation.
 * @param {string} dir - the slot worktree directory.
 */
export function breakTurn(dir) {
  mutateDoc(dir, current => ({ ...current, turn: null }));
}

/**
 * Remove the worktree document and its transient artifacts entirely (including broken mutex
 * husks) - `slot rm`'s cleanup before `git worktree remove`, which refuses to remove a
 * worktree holding untracked files.
 * @param {string} dir - the slot worktree directory.
 */
export function removeDoc(dir) {
  let names = [];
  try {
    names = readdirSync(dir).filter(name => name === LOCK_FILENAME || name.startsWith(LOCK_TMP_FILENAME));
  }
  catch { /* dir gone */ }
 ...
 * cannot drift (a dropped workerLive arg misjudges every lock as live).
