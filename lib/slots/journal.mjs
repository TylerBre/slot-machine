...
export function appendJournal(repo, record) {
  ...
  try {
    ...
    // A concurrent rotator may have renamed the live file between our open and write,
    // landing this record in the rotated generation - readable now, but it would age out
    // one rotation early. Verify the fd still names the live path; on mismatch re-append
    // to the NEW live file (the possible duplicate is absorbed by readers' identity
    // dedup on ts+type+slot). The race itself is not interleavable in-process - this
    // branch is pinned by review, like the fsync above.
    try {
