// The per-repo turn journal: an append-only JSONL of fleet facts (workers created, tasks
// dispatched, turns started/completed). History, never a mailbox - the inbox stays the
// consumption channel. Durability and rotation rules: see README.md.
import { closeSync, existsSync, fstatSync, fsyncSync, mkdirSync, openSync, readFileSync, renameSync, statSync, unlinkSync, watch, writeSync } from 'node:fs';
 ...
 * Throws on invalid records and on IO failure - the CALLER owns the degrade policy.
...
export function appendJournal(repo, record) {
  ...
  try {
    ...
    // A concurrent rotator may have renamed the live file between open and write, landing
    // this record in the rotated generation (it would age out one rotation early). Verify
    // the fd still names the live path; on mismatch re-append to the NEW live file - the
    // possible duplicate is absorbed by readers' identity dedup on ts+type+slot.
    try {
      moved = fstatSync(fd).ino !== statSync(livePath(repo)).ino;
    }
    catch {
      moved = true; // live path vanished mid-append: rotation is mid-flight
    }
  }
  finally {
    closeSync(fd);
  ...
  if (moved) {
    const redoFd = openSync(livePath(repo), 'a');
    try {
      writeSync(redoFd, payload);
      fsyncSync(redoFd);
    }
    finally {
      closeSync(redoFd);
...
export function readJournal(repo, { sinceTs, tail } = {}) {
  ...
  // >= not >: a same-ms tie straddling rotation (rotated tail and live head share a ts)
  // must consult the rotated generation or the older twin is silently skipped.
  const needRotated
