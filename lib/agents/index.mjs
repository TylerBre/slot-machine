...
// slot to a concrete { plugin, model, env }. Pure helpers carry the decision logic;
// loadRoster/resolveInstance wrap them with config + dynamic-import IO (see README.md).
...
// Pure (no loaded plugin): models/defaultModel declared only in a user plugin's code read as
// open ('*')/null here - declare them in the roster entry (agents add --models) to enforce.
...
export function resolveModel(cfg, repoDir, label, name) {
  ...
  if (slot.model != null && slotAgent === name)
    ...
    // repoAgent === name covers plain inheritance AND a slot that redundantly restates the repo's agent
    model = repo.model;
