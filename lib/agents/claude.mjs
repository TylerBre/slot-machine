...
// PROJECT's settings (<deskDir>/.claude/settings.json - never user-level). The installed
// command is REPO-PINNED (`--repo <repoDir>`): an unpinned command gates against the
// globally mutable current-repo pointer, so `sm repo use` in any terminal would silently
// repoint every desk's hooks at another repo's state. Dedup is by exact command string,
// the same rule Claude Code itself uses when merging hook levels; setup upgrades a bare
// (unpinned) command to the pinned form in place.
