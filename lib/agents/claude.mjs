...
// --- delivery capability (optional op pair; see the supervision spec) --------------------
// The watch's hook delivery for Claude Code: a Stop + a UserPromptSubmit hook in the DESK
// PROJECT's settings (<deskDir>/.claude/settings.json - never user-level), each invoking the
// seat-gated `sm watch --check --ack --hook <type>`. Dedup is by exact command string, the
// same rule Claude Code itself uses when merging hook levels.
const HOOK_COMMANDS = {
  Stop: 'sm watch --check --ack --hook stop',
  UserPromptSubmit: 'sm watch --check --ack --hook prompt-submit',
};
const deskSettingsPath = deskDir => join(deskDir, '.claude', 'settings.json');

function deliveryWired(deskDir) {
  let settings;
  try {
    settings = JSON.parse(readFileSync(deskSettingsPath(deskDir), 'utf8'));
  }
  catch {
    return { stop: false, promptSubmit: false };
  }
  const has = event => (settings.hooks?.[event] ?? [])
    .some(matcher => (matcher.hooks ?? []).some(hook => hook.command === HOOK_COMMANDS[event]));
  return { stop: has('Stop'), promptSubmit: has('UserPromptSubmit') };
}
...
export default {
  ...
  doctor({ env, mcpServers, deskDir }) {
    ...
    // Delivery status folds into the doctor report (additive) when the caller says where
    // the desk project lives.
    return ok(deskDir ? { version, mcp, delivery: deliveryWired(deskDir) } : { version, mcp });
  },

  deliverySetup({ deskDir }) {
    if (!deskDir)
    ...
    if (existsSync(path)) {
      try {
        ...
        // Never rewrite what we cannot faithfully read - a clobbered settings file would
        // eat the user's own hooks/permissions.
    ...
    for (const [event, command] of Object.entries(HOOK_COMMANDS)) {
      ...
      const present = matchers.some(matcher => (matcher.hooks ?? []).some(hook => hook.command === command));
      if (present)
        continue; // idempotent: dedup by exact command string
      matchers.push({ matcher: '*', hooks: [{ type: 'command', command, timeout: 30 }] });
    ...
    return ok({ installed, path, changed: installed.length > 0 });
  },
